...
async function getLiquidityData(poolAddress) {
  ...
  console.log(`Current price: ${tickToPrice(currentTickNumber, 'USDC', 'ETH')} USDC per ETH`);
  
  const ticks = [];
  let leftTick = Math.floor(currentTickNumber / tickSpacing) * tickSpacing;
  let rightTick = leftTick + tickSpacing;
  
  console.log("Searching for ticks with liquidity...");
  
  while (ticks.length < 10) { // Search until we find 10 ticks with liquidity
    // Check left tick
    const leftTickInfo = await pool.ticks(leftTick);
    if (leftTickInfo.liquidityNet.toString() !== '0') {
      const price = tickToPrice(leftTick, 'USDC', 'ETH');
      console.log(`Tick ${leftTick}: Liquidity = ${leftTickInfo.liquidityNet.toString()}, Price = ${price.toFixed(2)} USDC per ETH`);
      ticks.push({ 
        tick: leftTick.toString(), 
        liquidityNet: leftTickInfo.liquidityNet.toString(),
        price: price.toFixed(2)
      });
    }
    
    // Check right tick
    const rightTickInfo = await pool.ticks(rightTick);
    if (rightTickInfo.liquidityNet.toString() !== '0') {
      const price = tickToPrice(rightTick, 'USDC', 'ETH');
      console.log(`Tick ${rightTick}: Liquidity = ${rightTickInfo.liquidityNet.toString()}, Price = ${price.toFixed(2)} USDC per ETH`);
      ticks.push({ 
        tick: rightTick.toString(), 
        liquidityNet: rightTickInfo.liquidityNet.toString(),
        price: price.toFixed(2)
      });
    }
    
    leftTick -= tickSpacing;
    rightTick += tickSpacing;
    
    // Safety check to prevent infinite loop
    if (Math.abs(leftTick - currentTickNumber) > 1000000) {
      console.log("Reached search limit. Stopping.");
      break;
    }
  }
  
  console.log(`Total ticks with liquidity found: ${ticks.length}`);
  return ticks;
}
...
function tickToPrice(tick, baseToken, quoteToken) {
  const sqrtPrice = Math.sqrt(1.0001 ** tick);
  let price = sqrtPrice ** 2;

  // Adjust for decimals
  const baseDecimals = baseToken === 'USDC' ? 6 : 18; // USDC has 6 decimals, ETH has 18
  const quoteDecimals = quoteToken === 'USDC' ? 6 : 18;
  price = price * (10 ** (quoteDecimals - baseDecimals));
